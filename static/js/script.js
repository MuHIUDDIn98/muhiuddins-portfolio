document.addEventListener('DOMContentLoaded', () => {
    ...
    navMenuLinks.forEach(link => {
        link.addEventListener('click', () => {
            ...
            if (href && href.startsWith('#') && navMenu.classList.contains('is-active')) {
                toggleMenu();
            }
        });
    });
    ...
    const filterSkills = (category) => {
        subTabBtns.forEach(btn => btn.classList.toggle('active', btn.dataset.category === category));
    ...
    const filterProjects = (filter) => {
        projectFilterBtns.forEach(btn => btn.classList.toggle('active', btn.dataset.filter === filter));
    ...
    const filterLicenses = (filter) => {
        licenseFilterBtns.forEach(btn => btn.classList.toggle('active', btn.dataset.filter === filter));
    ...
    // =================================================================
    // 3. INTERACTIVE & SCROLL-BASED EFFECTS
    // =================================================================
    ...
    // --- Background Click Tracking for Email Button ---
    const emailButton = document.getElementById('track-email-click');
    if (emailButton) {
    ...
    // --- General Scroll-In Animation (for non-filtered items) ---
